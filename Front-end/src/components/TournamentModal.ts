...
// Removed unused function showBracketWithBackendMatches
...
function showBracket(players: string[]) {
  ...
  // Removed unused originalSelectWinner
  // Removed unused selectWinnerFunc
  ...
  async function startGame(playerA: string, playerB: string, matchNum: string | null) {
    ...
    try {
      ...
      if (matchNum === '1' && backendMatches?.[0]) {
        ...
        if (backendMatches?.[2]) {
          ...
          try {
            ...
            if (finalMatch) {
              ...
              // Final match doesn't exist yet - try to trigger its creation
              console.log('[DEBUG] Final match not found, attempting to trigger creation...');
              console.log('[DEBUG] Available matches structure:', JSON.stringify(latestMatches, null, 2));
              
              // Check if both semifinals are actually completed by checking match results
              // Try different possible field names for match identification
              const semifinal1 = latestMatches.find((match: any) => 
                (match.round === 'semifinal' && match.matchNumber === 1) ||
                (match.matchId === latestMatches[0]?.matchId) ||
                (match.id === latestMatches[0]?.id)
              );
              const semifinal2 = latestMatches.find((match: any) => 
                (match.round === 'semifinal' && match.matchNumber === 2) ||
                (match.matchId === latestMatches[1]?.matchId) ||
                (match.id === latestMatches[1]?.id)
              );
              
              console.log('[DEBUG] Semifinal 1 data:', JSON.stringify(semifinal1, null, 2));
              console.log('[DEBUG] Semifinal 2 data:', JSON.stringify(semifinal2, null, 2));
              
              // Check for completion using different possible field names
              const semifinal1Complete = semifinal1 && (
                semifinal1.winner || 
                semifinal1.winnerId || 
                semifinal1.status === 'completed' ||
                semifinal1.finished === true ||
                (semifinal1.player1Score !== undefined && semifinal1.player2Score !== undefined)
              );
              
              const semifinal2Complete = semifinal2 && (
                semifinal2.winner || 
                semifinal2.winnerId || 
                semifinal2.status === 'completed' ||
                semifinal2.finished === true ||
                (semifinal2.player1Score !== undefined && semifinal2.player2Score !== undefined)
              );
              
              console.log('[DEBUG] Semifinals completion status:', {
                semifinal1Complete,
                semifinal2Complete,
                semifinal1Fields: semifinal1 ? Object.keys(semifinal1) : 'not found',
                semifinal2Fields: semifinal2 ? Object.keys(semifinal2) : 'not found'
              });
              
              if (semifinal1Complete && semifinal2Complete) {
                console.log('[DEBUG] Both semifinals completed, trying to create final match...');
                
                // Try calling tournament start again to trigger final match creation
                try {
                  const { apiService } = await import('../services/api');
                  await apiService.tournaments.start(tournamentId);
                  console.log('[DEBUG] Called tournament start to trigger final match creation');
                  
                  // Fetch matches again to see if final was created
                  const updatedMatches = await fetchTournamentMatches(tournamentId);
                  const newFinalMatch = updatedMatches.find((match: any) => match.round === 'final');
                  
                  if (newFinalMatch) {
                    matchId = newFinalMatch.matchId;
                    console.log('[DEBUG] Successfully created and found final match with ID:', matchId);
                    // Update global storage
                    (window as any).globalBackendMatches = updatedMatches;
                  } else {
                    throw new Error('Failed to create final match - backend issue');
                  }
                } catch (startError) {
                  console.error('[DEBUG] Error triggering final match creation:', startError);
                  throw new Error('Final match not yet available - complete both semifinals first');
                }
              } else {
                console.log('[DEBUG] Semifinals not completed yet:', {
                  semifinal1: semifinal1?.winner ? 'completed' : 'pending',
                  semifinal2: semifinal2?.winner ? 'completed' : 'pending'
                });
                throw new Error('Final match not yet available - complete both semifinals first');
              }
            }
        ...
        throw new Error(`No matchId found for match ${matchNum}. Tournament backend integration failed.`);
      }
            ...
            setTimeout(() => {
              if (gamePage.game) {
                ...
                if (player1Score >= 5 || player2Score >= 5) {
                  ...
                  (async () => {
                    try {
                      // const apiModule = await import('../services/api.js'); // Commented out unused import
                      // const apiService = apiModule.apiService; // Commented out unused variable
  ...
  if ((window as any).globalMatchResults) {
    ...
    // Update Final winner
    if (results['final']) {      const finalElement = bracket.querySelector('[data-match="final"]');
      if (finalElement) {
        const winnerDiv = document.createElement('div');
  ...
  function selectWinner(playerElement: HTMLElement, matchNumber: string | number) {
    // Manual selection disabled - winners are determined automatically by game results    return;
    // Store winner
