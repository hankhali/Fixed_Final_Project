...
export function navigateTo(path: string): void {
  ...
  showLoading();
  setTimeout(() => {
    console.log('[Navigation] Processing navigation to:', path);
    window.history.pushState({}, "", path);
    const app = document.getElementById("app");
    if (app) {
      console.log('[Navigation] Setting up routes for:', path);
      setupRoutes(app);
      // Update active navbar link
      // updateActiveNavLink(path); // TODO: Fix this function
      // Update page title
      document.title = getPageTitle(path);
      console.log('[Navigation] Navigation completed for:', path);
    }
    hideLoading();
...
// Function to check for Google OAuth token in URL hash
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  console.log('[App] About to call navigateTo...');
  ...
  try {
    if (window.location.pathname === '/' || window.location.pathname === '') {
      ...
      console.log('[App] Navigating to current path:', window.location.pathname);
      navigateTo(window.location.pathname);
    }
    console.log('[App] navigateTo call completed');
  } catch (error) {
    console.error('[App] Error calling navigateTo:', error);
  }

  console.log('[App] Initialization complete');
  
  // Fallback: Hide loading overlay after initialization
  setTimeout(() => {
    console.log('[App] Fallback: Hiding loading overlay');
    hideLoading();
  }, 1000);
});
